...
/// Debug-only override: `LIQUENT_BLACKHOLE_BROADCAST=1` forces this node to keep
...
pub fn resolve_enable_broadcast(role_is_fullnode: bool) -> bool {
    if std::env::var("LIQUENT_BLACKHOLE_BROADCAST").as_deref() == Ok("1") {
        tracing::warn!(
            "LIQUENT_BLACKHOLE_BROADCAST=1: mempool broadcast forcibly \
