...
Command-line tool for interacting with the Liquent chain. Provides commands for genesis setup, validator management, stake operations, node lifecycle, and DKG queries.
...
```bash
liquent_cli genesis generate-account \
  --output-file <path>         # Output YAML file path (required)
```
...
liquent_cli validator join \
  ...
  [--moniker <name>]                    # Display name, max 31 bytes (default: "Liquent1")
...
Start a Liquent node using the deployment's `script/start.sh`.

```bash
liquent_cli node start \
  --deploy-path <path>         # Deployment directory containing script/start.sh (required)
```

#### `node stop`

Stop a running Liquent node using the deployment's `script/stop.sh`.

```bash
liquent_cli node stop \
