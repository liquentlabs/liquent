...
fn spawn_rayon_thread_pool(name: String, num_threads: Option<usize>) -> rayon::ThreadPool {
    use laptos::aptos_runtimes;
    aptos_runtimes::spawn_rayon_thread_pool(name, num_threads)
}

fn poly_mul_less_slow(f: &[Scalar], g: &[Scalar]) -> Vec<Scalar> {
    poly_mul_fft(&f.to_vec(), &g.to_vec())
}
