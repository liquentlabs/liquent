...
fn test_wvuf_basic_viability() {
    weighted_wvuf_bvt::<das::WeightedTranscript, PinkasWUF>();
}
    ...
    WVUF::PublicParameters: for<'a> From<&'a T::PublicParameters>,
{
    ...
    // Do a weighted PVSS
    // StdRng::from_seed expects [u8; 32] as seed
    let seed_vec = seed.to_bytes_le();
    let mut seed_bytes = [0u8; 32];
    seed_bytes[..seed_vec.len().min(32)].copy_from_slice(&seed_vec[..seed_vec.len().min(32)]);
    let mut rng = StdRng::from_seed(seed_bytes);

    let (wc, d, trx) = weighted_pvss::<T, StdRng>(&mut rng);
...
fn weighted_pvss<T: Transcript<SecretSharingConfig = WeightedConfig>, R>(
    rng: &mut R,
