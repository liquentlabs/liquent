...
impl DirectMempoolQuorumStore {
    ...
    async fn pull_internal(
        ...
        let exclude_txns: BTreeMap<_, _> = exclude_txns
            ...
            .map(|txn| {
                (
                    laptos::aptos_consensus_types::common::TransactionSummary::new(
                        txn.sender,
                        txn.sequence_number,
                        txn.hash,
                    ),
                    laptos::aptos_consensus_types::common::TransactionInProgress::new(0),
                )
            })
            .collect();
    ...
    async fn handle_block_request(
        ...
        // TODO(liquent_byteyue): remove all codes
