...
use crate::{
    quorum_store::{
        ...
        types::{Batch, BatchKey, PersistedValue},
    },
    test_utils::create_vec_signed_transactions,
};
use aptos_consensus_types::proof_of_store::BatchId;
use claims::assert_ok;
use laptos::{aptos_temppath::TempPath, aptos_types::account_address::AccountAddress};
...
fn test_db_for_data() {
    ...
    assert_eq!(
        db.get_batch(&BatchKey::new(persist_request_1.epoch(), *persist_request_1.digest()))
    ...
    let batches = vec![BatchKey::new(persist_request_3.epoch(), *persist_request_3.digest())];
    assert_ok!(db.delete_batches(batches));
    assert_eq!(
        db.get_batch(&BatchKey::new(persist_request_3.epoch(), *persist_request_3.digest()))
            .expect("could not read from db"),
        None
    );
