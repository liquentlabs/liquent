...
impl BatchGenerator {
    pub(crate) fn new(
            ...
            db.clean_and_get_batch_id(epoch).expect("Could not read from db")
        {
            ...
            BatchId::new(laptos::aptos_infallible::duration_since_epoch().as_micros() as u64)
        };
    ...
    pub(crate) async fn handle_scheduled_pull(&mut self, max_count: u64) -> Vec<Batch> {
        ...
        let expiry_time = laptos::aptos_infallible::duration_since_epoch().as_micros() as u64 +
    ...
    pub(crate) fn handle_remote_batch(
        ...
        let expiry_time_usecs = laptos::aptos_infallible::duration_since_epoch().as_micros() as u64 +
