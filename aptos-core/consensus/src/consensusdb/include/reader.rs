use bytes::Bytes;
use laptos::api_types::config_storage::{
    OnChainConfig as LiquentOnChainConfig, GLOBAL_CONFIG_STORAGE,
};
use laptos::aptos_crypto::hash::ACCUMULATOR_PLACEHOLDER_HASH;
use laptos::aptos_storage_interface::{DbReader, DbWriter};
use laptos::aptos_types::epoch_change::EpochChangeProof;
use laptos::aptos_types::ledger_info::LedgerInfoWithSignatures;
use laptos::aptos_types::on_chain_config::{ConsensusAlgorithmConfig, ProposerElectionType};
use laptos::aptos_types::on_chain_config::{OnChainConfig, ValidatorSet};
use laptos::aptos_types::state_proof::StateProof;
use laptos::aptos_types::state_store::state_key::inner::StateKeyInner;
use laptos::aptos_types::{
        ...
        VALIDATOR_SET
            .get_or_init(|| {
                let validator_set_config = GLOBAL_CONFIG_STORAGE
                    .get()
                    .unwrap()
                    .fetch_config_bytes(LiquentOnChainConfig::ValidatorSet, 0.into());
                let validator_bytes =
...
// TODO(liquent_byteyue): this is a temporary solution to enable quorum store
            ...
            match key {
                StateKeyInner::AccessPath(p) => {
                    ...
                    if path.contains("Validator") {
                        ...
                        // todo(liquent_byteyue): currently we set quorum_store_enabled=false
