...
use crate::{
    ...
    liquent_state_computer::{ConsensusAdapterArgs, LiquentBlockExecutor},
    network::NetworkTask,
...
pub fn start_consensus(
    ...
    liquent_args: &mut ConsensusAdapterArgs,
) -> (Runtime, Arc<StorageWriteProxy>, Arc<QuorumStoreDB>) {
    let runtime = laptos::aptos_runtimes::spawn_named_runtime("consensus".into(), None);
    let storage = Arc::new(StorageWriteProxy::new(
        liquent_args.consensus_db.as_ref().unwrap().clone(),
    ...
    let g_executor = LiquentBlockExecutor::new(
    ...
    let (timeout_sender, timeout_receiver) =
        laptos::aptos_channels::new(1_024, &counters::PENDING_ROUND_TIMEOUTS);
    let (self_sender, self_receiver) =
        laptos::aptos_channels::new_unbounded(&counters::PENDING_SELF_MESSAGES);
...
pub fn start_consensus_observer(
    ...
    let runtime = laptos::aptos_runtimes::spawn_named_runtime("observer".into(), None);
