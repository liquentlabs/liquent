...
pub struct ExecutionProxy {
    ...
    async_state_sync_notifier: laptos::aptos_channels::Sender<NotificationType>,
...
impl ExecutionProxy {
    ...
    pub fn new(
        ...
        let (tx, mut rx) = laptos::aptos_channels::new::<NotificationType>(
            10,
    ...
    fn transactions_to_commit(
        ...
        // TODO(liquent_byteyue): We manually skipped the validator txns and metadata here.
    ...
    fn process_single_validator_transaction(
        ...
        match txn {
            ValidatorTransaction::DKGResult(transcript) => {
                let transcript = laptos::api_types::on_chain_config::dkg::DKGTranscript {
                    metadata: laptos::api_types::on_chain_config::dkg::DKGTranscriptMetadata {
    ...
    /// Process JWK update and convert to laptos format
    fn process_jwk_update(&self, update: &ProviderJWKs, block: &Block) -> Option<Vec<u8>> {
        use laptos::api_types::on_chain_config::jwks::{JWKStruct, ProviderJWKs};
        // TODO(Liquent): Check the signature here instead of execution layer
        info!(
        ...
        let laptos_provider_jwk = ProviderJWKs {
        ...
        bcs::to_bytes(&laptos_provider_jwk)
            .map_err(|err| {
...
impl StateComputer for ExecutionProxy {
    async fn schedule_compute(
        ...
        let real_txns: Vec<laptos::api_types::VerifiedTxn> = vtxns
            .into_iter()
            .map(|txn| {
    ...
    async fn commit(
        ...
        // TODO(liquent_lightman): The take_pre_commit_fut will cause a coredump.
