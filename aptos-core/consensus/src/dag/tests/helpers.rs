...
use laptos::aptos_types::{
    aggregate_signature::AggregateSignature, transaction::SignedTransaction,
};
    ...
    for (round, round_nodes) in dag.iter().enumerate() {
        ...
        for (idx, author) in validators.iter().enumerate() {
            if let Some(bitmask) = &round_nodes[idx] {
                // the bitmask is compressed (without the holes), we need to flatten the previous
                // round nodes to match the index
                ...
                nodes_at_round.push(Some(new_certified_node((round + 1) as u64, *author, parents)));
            } else {
