...
impl DbReader for MockDbReader {
    fn get_latest_block_events(
        &self,
        num_events: usize,
    ) -> laptos::aptos_storage_interface::Result<Vec<EventWithVersion>> {
        *self.fetched.lock() += 1;
    ...
    fn get_latest_ledger_info_version(&self) -> laptos::aptos_storage_interface::Result<Version> {
    ...
    fn get_accumulator_root_hash(
        &self,
        _version: Version,
    ) -> laptos::aptos_storage_interface::Result<HashValue> {
        Ok(HashValue::zero())
    }
}
