        ...
        self.execution.as_ref().and_then(|config| {
            ...
            if sizes.len() >= lookback_config.min_blocks_to_activate {
                let percentile = match lookback_config.metric {
                    laptos::aptos_config::config::ExecutionBackpressureMetric::Mean => 0.5,
                    laptos::aptos_config::config::ExecutionBackpressureMetric::Percentile(
                        percentile,
                    ) => percentile,
                };
...
impl ProposalGenerator {
    ...
    pub async fn generate_proposal(
        ...
        let (validator_txns, payload, timestamp) = if hqc.certified_block().has_reconfiguration() {
            ...
            // TODO(liquent_byteyue): Consider how to process the validator transaction
