...
use laptos::{aptos_consensus::counters::WAIT_FOR_FULL_BLOCKS_TRIGGERED, aptos_logger::info};
...
impl UserPayloadClient for QuorumStoreClient {
    async fn pull(
        ...
        let payload = loop {
            ...
            // max_poll_time; TODO(liquent_byteyue): maybe we don't need to set true as
            // default
            let done = true;
            let payload = self
                .pull_internal(
