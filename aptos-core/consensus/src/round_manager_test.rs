...
impl NodeSetup {
    fn create_round_state(time_service: Arc<dyn TimeService>) -> RoundState {
        ...
        let (round_timeout_sender, _) = laptos::aptos_channels::new_test(1_024);
    ...
    async fn create_nodes(
        ...
        for (id, signer) in signers.iter().take(num_nodes).enumerate() {
            ...
            let safety_storage = PersistentSafetyStorage::initialize(
                Storage::from(laptos::aptos_secure_storage::InMemoryStorage::new()),
                signer.author(),
    ...
    fn new(
        ...
        let (self_sender, self_receiver) = laptos::aptos_channels::new_unbounded_test();
        let network =
