...
impl PipelinedBlock {
    ...
    pub fn subscribable_events(&self) -> Vec<ContractEvent> {
        ...
        // TODO(liquent_byteyue): we should have a better way to identify reconfiguration suffix
        // reconfiguration event之后的block是不应该被执行的. 在liquent-sdk的模型中暂时不会有这个问题
        // 但是可能需要仔细考虑
...
mod set_randomness_tests {
    use super::*;
    use laptos::aptos_types::randomness::RandMetadata;
