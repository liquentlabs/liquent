...
# liquent_cli reports EVERY reverted staking tx with one of these catch-all
...
class Cluster:
    """
    Unified entry point for interacting with a Liquent Cluster.
    ...
    def __init__(self, config_path: Path):
        ...
        self.liquent_cli_path = self.base_dir / "liquent_cli"
    ...
    def get_bench_accounts(self, limit: Optional[int] = None) -> List[Account]:
        ...
        if not artifacts_dir:
            raise RuntimeError(
                "LIQUENT_ARTIFACTS_DIR environment variable is not set. "
                "This is required to locate accounts.csv generated by liquent_bench."
            )
    ...
    async def _run_cli_with_staking_retry(
        ...
        """
        Run a liquent_cli command, retrying failures that match the CLI's
    ...
    async def create_stake(
        ...
        create_cmd = [
            ...
            "--stake-amount",
            stake_amount,
            # StakePool deployment (~9KB runtime code) exceeds the liquent_cli
