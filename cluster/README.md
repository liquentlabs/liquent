# Liquent Cluster Management Tools

This directory contains utility scripts to initialize, deploy, and manage a local Liquent Devnet cluster.
...
> ⚠️ **Important**: The `genesis.sh` script will clone `liquent_chain_core_contracts` to `external/` if it doesn't exist, but will **NOT** automatically update it. To update:
> ```bash
> cd external/liquent_chain_core_contracts && git pull origin main
> ```
...
Prepares the runtime environment (default: `/tmp/liquent-cluster`).
*   **Creates hardlinks** for `liquent_node` and `liquent_cli` binaries
...
| `[build]` | Path to `liquent_node` binary |
