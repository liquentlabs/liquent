...
use aptos_consensus::{consensusdb::ConsensusDB, liquent_state_computer::ConsensusAdapterArgs};
...
impl ConsensusEngine {
    pub async fn init(
        ...
        let ConsensusEngineArgs { node_config, chain_id, latest_block_number, config_storage } =
            args;
        // Setup panic handler
        laptos::aptos_crash_handler::setup_panic_handler();
        ...
        let mut event_subscription_service =
            laptos::aptos_event_notifications::EventSubscriptionService::new(Arc::new(
                laptos::aptos_infallible::RwLock::new(db.clone()),
            ));
        ...
        for network_config in network_configs.into_iter() {
            ...
            let mut network_builder = NetworkBuilder::create(
                ...
                &network_config,
                laptos::aptos_time_service::TimeService::real(),
        ...
        // consensus_to_mempool_sender in Liquent
        let (consensus_notifier, consensus_listener) =
            laptos::aptos_consensus_notifications::new_consensus_notifier_listener_pair(
        ...
        let mempool_listener =
            laptos::aptos_mempool_notifications::MempoolNotificationListener::new(
        ...
        // For Liquent we only use it to notify the mempool for the committed txn gc logic
        let mempool_notifier =
            laptos::aptos_mempool_notifications::MempoolNotifier::new(notification_sender);
        ...
        let runtime = laptos::aptos_runtimes::spawn_named_runtime("Con2Mempool".into(), None);
        ...
        #[cfg(debug_assertions)]
        {
            ...
            if !https_config.address.is_empty() {
                let runtime = laptos::aptos_runtimes::spawn_named_runtime("Http".into(), None);
                runtime.spawn(async move {
                    https_server(
